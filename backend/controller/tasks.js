...
const TwitterLogin = async (req, res) => {
  ...
  try {
    const { url, oauth_token, oauth_token_secret } =
      await requestClient.generateAuthLink(process.env.TWITTER_CALLBACK_URL);
  ...
  try {
    ...
    if (!check?.error) {
      const update = await User.findOneAndUpdate(
        { wallet: wallet },
        {
          $set: { isDiscordServer: true },
          $inc: { tokens: 4, totalTokensEarned: 4 },
        }
      );
      if (!update) {
        console.log("Error updating user");
    ...
    const checkWhetherUserLoggedIn = await User.findOne({
      wallet: wallet,
      isAlreadyLoggedIn: true,
    });
    if (!checkWhetherUserLoggedIn) {
      await User.findOneAndUpdate(
        { wallet: wallet },
        {
          $set: { isAlreadyLoggedIn: true },
          $inc: { tokens: 10, totalTokensEarned: 10 },
        }
      );
    }
    return await res.redirect(data);
    ...
    switch (Number(taskID)) {
      case 1: {
        ...
        const update = await User.findOneAndUpdate(
          { wallet: wallet },
          {
            $set: { isTwitterFollowed: true },
            $inc: { tokens: 3, totalTokensEarned: 3 },
          }
        );
        if (!update) {
          return res.status(400).json({
      ...
      case 2: {
        ...
        const check = await DiscordGuildCheck(wallet, "635865020172861441");
        if (check?.error) {
          return res.status(400).json({ error: check?.error });
        }
        const update = await User.findOneAndUpdate(
      ...
      case 4: {
        ...
        const upUser = await User.findOneAndUpdate(
          { wallet: wallet },
          {
            $inc: { tokens: 6, totalTokensEarned: 6 },
            $push: { referralUsed: referralCode },
          }
        );
        const incCredits = await User.findOneAndUpdate(
          { referralCode: referralCode },
          { $inc: { tokens: 6, totalTokensEarned: 6 } }
        );
        if (!upUser || !incCredits) {
      ...
      case 5: {
        if (
          ...
          const check = await TwitterTweetCheck(wallet);
          console.log(check);
          if (check?.error) {
            return res.status(400).json({ error: check?.error });
          }
          await User.findOneAndUpdate(
            { wallet: wallet },
            {
              $set: { socialLinkShareTime: new Date() },
              $inc: { tokens: 5, totalTokensEarned: 5 },
            }
          );
          return res.status(200).json({
          ...
          return res
            .status(400)
            .json({ error: "Please come back after 24 hours" });
        }
      }
      default:
