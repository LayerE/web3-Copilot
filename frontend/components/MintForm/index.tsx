...
import { useAccount, useWalletClient, useSwitchNetwork, useNetwork } from "wagmi";
...
import { ABI,ADDRESS,ERC1155ABI,ERC1155ADDRESS,ZK721,ZK1155 } from '@/utils/contractData';
...
const MintForm = ({ formData,prompt }: { formData: any, prompt: any}) => {
  console.log(formData);
  const [selectedUserImg, setSelectedUserImg] = React.useState<imgProp>({
    userImg: null,
    userImgPrev: null,
  });
  const { isLoggedIn,removeMintHistory,updateCurrentSession } = useChatStore();
  ...
  const MintNFT = async ({
    ...
    try {
      let Chain = chain?.toLocaleLowerCase()?.includes("pos") ? 137 : 1101;
      setLoading(true);
      ...
      if (res.status === 200) {
        ...
        const contract = new web3.eth.Contract(ABI as any,Chain === 137 ? ADDRESS : ZK721);
        const mint = await contract?.methods?.Mint(address,data?.metadata)?.send({
          from: address
        });
        console.log(mint)
        if(mint){
        ...
        const contract = new web3.eth.Contract(ERC1155ABI as any,Chain === 137 ? ERC1155ADDRESS : ZK1155);
        const mint = await contract?.methods?.mint(address,data?.metadata,1)?.send({
          from: address
        });
        if(mint){
          console.log(mint)
  ...
  return (
    <MintFormWrapper>
      <p>Now Upload your NFT image.</p>
      <FormWrapper>
        <ImageUploader
          onChange={userImgUpload}
          src={String(selectedUserImg?.userImgPrev)}
        />
        {/* <ConnectKitButton /> */}
          <Button
            disabled={
